...
from sqlalchemy import create_engine, Column, Integer, String, Float, Boolean, DateTime, ForeignKey, Text, or_, func, text
...
class User(Base):
    ...
    def is_admin(self):
        """User is admin if they have no manager (top of hierarchy)"""
        return self.manager_id is None
    
    def is_manager(self, db):
        """User is manager if they have direct reports"""
        return db.query(User).filter(User.manager_id == self.id).count() > 0
...
class KPI(Base):
    ...
    type = Column(String)  # global, employee-specific
...
class Evaluation(Base):
    ...
    status = Column(String, default="draft")  # draft, self_submitted, manager_review, admin_review, approved, rejected
    comments = Column(Text)
    manager_comments = Column(Text)  # Manager-only comments
    admin_comments = Column(Text)  # Admin-only comments
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    submitted_at = Column(DateTime)  # When the evaluation was submitted by employee
    manager_reviewed_at = Column(DateTime)  # When reviewed by manager
    approved_at = Column(DateTime)  # When the evaluation was approved by admin
    ...
    current_reviewer_id = Column(Integer, ForeignKey("users.id"))  # Who should review next
...
def has_hierarchy_permission(db: Session, user_id: int, permission_type: str, target_user_id: int = None) -> bool:
    """Check if a user has hierarchy-based permission"""
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        return False
    
    # Admin (no manager) has all permissions
    if user.is_admin():
        return True
    
    # Manager permissions
    if permission_type == "manage_team":
        # Can manage direct reports
        if target_user_id:
            target_user = db.query(User).filter(User.id == target_user_id).first()
            return target_user and target_user.manager_id == user_id
        return user.is_manager(db)
    
    # Employee permissions
    if permission_type == "view_own_data":
...
def get_users(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if current_user.is_admin():
        ...
        # Manager can see their direct reports and themselves
